...
defineRule("water_control", {
    ...
    then: function(newValue, devName, cellName) {
        dev["wb-mwac_68"]["K1"] = newValue;
      	dev["wb-mwac_68"]["K2"] = newValue;
...
defineRule("water_fail_control", {
    ...
    then: function(newValue, devName, cellName) {
      if(newValue){
        ...
        dev["wb-mwac_68"]["Alarm"] = false;      
      }
    }
});

defineRule("water_button_control", {
    whenChanged: "wb-gpio/MOD1_IN1",
    then: function(newValue, devName, cellName) {
      if(newValue){
        if(dev["wb-mwac_68"]["Alarm"]){
